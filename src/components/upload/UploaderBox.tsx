...
interface UploaderBoxProps {
  ...
  description: string;
  aspectRatio?: string;
  enableHostUpload?: boolean; // Enable upload to PostImages
  onHostedUrlChange?: (url: string) => void; // Callback khi có URL từ host
}
    ...
    <div className="flex flex-col gap-2">
      ...
      {inputMode === 'upload' && (
        <div
          ...
          className={`relative w-full ${aspectRatio} bg-[var(--bg-secondary)] rounded-lg flex items-center justify-center transition-colors duration-200 select-none ${
      ...
      {/* Image URL mode with preview */}
      {inputMode === 'url' && imageUrl && (
        <div className={`relative w-full ${aspectRatio} bg-[var(--bg-secondary)] rounded-lg`}>
          <img src={imageUrl} alt={title} className="w-full h-full object-contain rounded-lg" />
          <button
            onClick={handleClear}
            className="absolute top-2 right-2 z-10 p-1 bg-black/50 backdrop-blur-sm rounded-full text-white hover:bg-red-600 transition-colors"
