...
const ImageEditorCanvas: React.FC<ImageEditorCanvasProps> = ({
  ...
  const doDraw = (e: React.MouseEvent | React.TouchEvent) => {
    ...
    if (maskCtx) {
      ...
      maskCtx.strokeStyle = `${accentColor}b3`;
        ...
        {!initialImageUrl ? (
          <label htmlFor="file-upload" className="flex flex-col items-center justify-center text-[var(--text-tertiary)] cursor-pointer w-full h-full">
            ...
            <p className="mb-2 text-sm">
              <span className="font-semibold text-[var(--text-secondary)]">{t('imageEditor.upload')}</span> {t('imageEditor.dragAndDrop')}
            </p>
          ...
          <>
            ...
            <canvas
              ...
              style={{ zIndex: 3, touchAction: 'none', cursor: isMaskToolActive ? 'crosshair' : 'default' }}
      ...
      {initialImageUrl && isMaskToolActive && (
        <div className="p-3 bg-black/60 backdrop-blur-md rounded-lg flex flex-col gap-4 border border-[var(--border-primary)] animate-fade-in-fast">
          ...
          <div className="flex items-center gap-4">
            <label htmlFor="brush-size" className="text-sm font-medium text-[var(--text-primary)] whitespace-nowrap">
