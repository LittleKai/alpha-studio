                                        ...
                                        {reviews.map(review => (
                                            <div key={review._id} className="glass-card rounded-xl p-6">
                                                <div className="flex items-start gap-4">
                                                    {review.user?.avatar ? (
                                                        <img src={cdnFromUrl(review.user.avatar, 'w_128')} alt={review.user.name} className="w-12 h-12 rounded-full object-cover" loading="lazy" decoding="async" />
                                                    ) : (
                                ...
                                <div className="space-y-4">
                                    {instructorsToShow.map((ins, idx) => (
                                        <div key={idx} className="flex items-center gap-4">
                                            {ins.avatar ? (
                                                <img src={cdnFromUrl(ins.avatar, 'w_128')} alt={ins.name} className="w-14 h-14 rounded-full object-cover flex-shrink-0" loading="lazy" decoding="async" />
                                            ) : (
                                    ...
                                    {reviews.map(review => (
                                        <div key={review._id} className="p-4 bg-[var(--bg-secondary)] rounded-xl">
                                            <div className="flex items-start gap-3">
                                                {review.user.avatar ? (
                                                    <img src={cdnFromUrl(review.user.avatar, 'w_128')} alt={review.user.name} className="w-10 h-10 rounded-full object-cover" loading="lazy" decoding="async" />
                                                ) : (
