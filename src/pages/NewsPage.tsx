...
export default function NewsPage() {
    ...
    useEffect(() => {
        const load = async () => {
            try {
                setLoading(true);
                const res = await getArticles('news', 1, 50);
    ...
    return (
        <div className="min-h-screen bg-[var(--bg-primary)]">
            <SEOHead
                title={language === 'vi' ? 'Tin Tức — Alpha Studio' : 'News — Alpha Studio'}
                description={language === 'vi'
                    ? 'Cập nhật những tin tức mới nhất về các khóa học, sự kiện và xu hướng AI trong ngành sáng tạo.'
                    : 'Stay up to date with the latest news, events, and AI trends in the creative industry.'}
                path="/news"
            />
            {/* Hero Section */}
            <section className="relative py-20 px-6 text-center overflow-hidden">
                ...
                <div className="relative max-w-4xl mx-auto">
                    <h1 className="text-4xl md:text-5xl font-black text-[var(--text-primary)] mb-4">
                        {t('landing.news.heroTitle')}
                    </h1>
                    <p className="text-lg text-[var(--text-secondary)] max-w-2xl mx-auto">
                        {t('landing.news.heroDescription')}
                    </p>
                </div>
            </section>
            ...
            <section className="max-w-7xl mx-auto px-6 pb-20">
                {loading ? (
                    ...
                    <div className="text-center py-20">
                        <p className="text-[var(--text-secondary)]">{t('landing.news.noArticles')}</p>
                    </div>
                ) : (
